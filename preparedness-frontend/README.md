...
<!-- One-to-many connections(instances) with another model - 'person' has_many houses/cars, A connection with another model - 'house' belongs_to a person, and a 'car' belongs_to a person -->
...
<!-- created account entered base structure-->
...
(Foreign Keys person & house - belongs_to)
  Table car {
  id integer
  person_id integer [ref: > person.id]
  house_id integer [ref: > house.id]
  created_at timestamp
  updated_at timestamp
}

(Foreign Key person - belongs_to)
  Table house {
  id integer [ref: > person.id]
  person_id integer
  created_at timestamp
  updated_at timestamp
} -->
